...
namespace tw.com.essentialoil.Controllers.User.BackUser
{
    public class BackUserProfileController : Controller
    {
        ...
        {
            var table = from c in db.tUserProfile
                        select c;
            return View(table);
        ...
        {
            tUserProfile cust = (new dbShoppingForumEntities()).tUserProfile.FirstOrDefault(c => c.fUserId == fUserId);
            if (cust == null)
        ...
        {
            tUserProfile cust = db.tUserProfile.FirstOrDefault(t => t.fUserId == c.fUserId);
            if (cust != null)
            {
                cust.fName = c.fName;
                cust.fTel = c.fTel;
